...
class OpenAIEmbeddingModel(EmbeddingModel):
    ...
    def __init__(self, model_name="text-embedding-ada-002", api_key=None):
        ...
        if not self.client.api_key:
            raise ValueError(
                "OpenAI API key not found - Please set the OPENAI_API_KEY environment variable or pass it as an argument"
            )
    ...
    def encode(self, texts, **kwargs):
        ...
        for i in tqdm(
            range(0, len(texts), batch_size), desc="Getting OpenAI Embeddings"
        ):
...
class DynamicSampler:
    ...
    def __init__(
        self,
        data_file_path,
        few_shot_pool_path_1,
        embedding_model: EmbeddingModel,
        few_shot_pool_path_2=None,
    ):
        ...
        self.pool1_data, self.pool1_embeddings = self._load_and_embed(
            few_shot_pool_path_1
        )
        ...
        if few_shot_pool_path_2 and os.path.exists(few_shot_pool_path_2):
            ...
            self.pool2_data, self.pool2_embeddings = self._load_and_embed(
                few_shot_pool_path_2
            )
    ...
    def get_dynamic_few_shot_examples(self, k, pool_1_percentage=1.0):
        ...
        print(
            f"Finding top {k} examples per test question ({k1} from pool 1, {k2} from pool 2)..."
        )
        ...
        for i in tqdm(range(len(self.dataset)), desc="Mapping few-shot examples"):
            ...
            if k1 > 0 and self.pool1_index:
                p1_dists, p1_indices = self.pool1_index.search(
                    np.array([test_embedding]).astype("float32"), num_to_search
                )
                p1_candidates = [
                    (p1_dists[0][j], self.pool1_data[p1_indices[0][j]], "pool1")
                    for j in range(len(p1_dists[0]))
                ]

            p2_candidates = []
            if k2 > 0 and self.pool2_index:
                p2_dists, p2_indices = self.pool2_index.search(
                    np.array([test_embedding]).astype("float32"), num_to_search
                )
                p2_candidates = [
                    (p2_dists[0][j], self.pool2_data[p2_indices[0][j]], "pool2")
                    for j in range(len(p2_dists[0]))
                ]
            ...
            for dist, example, pool_name in p1_candidates + p2_candidates:
                ...
                if (
                    example_id not in best_candidates
                    or dist < best_candidates[example_id][0]
                ):
                    best_candidates[example_id] = (dist, example, pool_name)

            best_from_pool1 = sorted(
                [ex for _, ex, pool in best_candidates.values() if pool == "pool1"],
                key=lambda x: x["id"],
            )
            best_from_pool2 = sorted(
                [ex for _, ex, pool in best_candidates.values() if pool == "pool2"],
                key=lambda x: x["id"],
            )
