    ...
    for m in prompt_methods:
        for fs in icl_methods:
            ...
            for idx, row in enumerate(logits_data_all[key]["cal"]):
                print(f"row is: {row}")
                opts = row.get("choices") or list(cal_raw[idx]["choices"].keys())
                logprobs = row["logprobs_options"]
                probs = np.exp(logprobs)  # shape (num_choices,)
                ...
                try:
                    ...
                    continue
            print(f"cal_scores is {cal_scores}")
